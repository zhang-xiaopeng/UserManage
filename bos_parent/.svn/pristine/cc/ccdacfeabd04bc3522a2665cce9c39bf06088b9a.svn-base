...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btnEp").click(function(){
			//进行表单校验
			var v = $("#editPasswordForm").form("validate") ;  //校验表单的长度是否一致
			if(v){
				//表单校验通过，手动校验两次输入的是否一致
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("系统提示","管理员邮箱: 1498051123@qq.com");
	}
</script>
</head>
<body class="easyui-layout">
	<div data-options="region:'north',border:false" style="height:80px;padding:10px;background:url('./images/header_bg.png') no-repeat right;">
		<div id="sessionInfoDiv" style="position: absolute;right: 5px;top:10px;">
			[<strong>${ loginUser.username }</strong>]，欢迎您！
		</div>
            ...
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
            	<form id="editPasswordForm">
	                <table cellpadding=3>
	                    <tr>
	                        <td>新密码：</td>
	                        <td><input id="txtNewPass" type="Password" class="txt01 easyui-validatebox" required="required" data-options="validType:'length[5,10]'" /></td>
	                    </tr>
	                    <tr>
	                        <td>确认密码：</td>
	                        <td><input id="txtRePass" type="Password" class="txt01 easyui-validatebox" required="required" data-options="validType:'length[5,10]'" /></td>
	                    </tr>
	                </table>
                </form>
            </div>
